...
function allocBlock(c) {
  ...
  return head + `
    ...
    <p class="hint"><b>在固定的波動目標下，「最佳報酬/波動」本來就是報酬最大的組合</b>
      ...
      <b>位階</b>是「離三個月高點幾個標準差」。算法：取最近 63 個交易日的日對數報酬，
      分母 = 這 63 天的標準差 × √63（＝三個月的一個標準差），
      分子 = 今天收盤相對這 63 天內最高收盤的對數報酬（≤ 0，今天就是高點時為 0）。
      跌 20% 在月波動 10% 的股票上是 −1.2σ，
      但指數只跌 10%、月波動 4%，卻是 −1.4σ，其實更極端——**用百分比比不同標的會比錯**。
      2026-09-23 之前這個數字算反了（量的是離最低點多遠），漲最多的反而最負。<br>
